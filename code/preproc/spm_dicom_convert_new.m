function out = spm_dicom_convert_new(hdr,opts,root_dir,format,out_dir)
...
%==========================================================================
function fnames = convert_standard(hdr,root_dir,format,out_dir)
[hdr,dist] = sort_into_volumes(hdr);
fnames = cell(length(hdr),1);
for i=1:length(hdr)
    fnames{i} = write_volume(hdr{i},root_dir,format,out_dir,dist{i});
end
...
%==========================================================================
function [vol,dist] = sort_into_volumes(hdr)
...
%==========================================================================
% function fname = write_volume(hdr,root_dir,format,out_dir,dist)
%==========================================================================
function fname = write_volume(hdr,root_dir,format,out_dir,dist)
...
% if cnt > 0
%     fprintf('***************************************************\n');
%     fprintf('* Removed %i redundant slices.                     *\n',cnt);
%     fprintf('***************************************************\n');    
% end
...
dicom_to_patient = [y1 y2 R]/[x1 x2 eye(4,2)]; % Shearing is taken into account here
mat              = patient_to_tal*dicom_to_patient*analyze_to_dicom;
...
%--------------------------------------------------------------------------
% Adjust for variable slice thickness
if sum((dist-mean(dist)).^2)/length(dist)>1e-4
    ...
    nvx = [ovx(1) ovx(2) 1]; % Set new slice thickness to 1 mm
...
%     % Create deformations
%     x0 = 1:nvx(2)/ovx(2):dim(2);
%     y0 = 1:nvx(1)/ovx(1):dim(1);
%     
%     z0  = 1;
%     tmp = 1;
%     cnt = 2;
%     for i=1:numel(dist)    
%         while tmp + 1/dist(i) <= i + 1
%             z0(cnt) = tmp + nvx(3)/dist(i);
%             tmp     = z0(cnt);
%             cnt     = cnt + 1;
%         end
%     end
...
%     % Resample image 
%     bsd                 = [1 1 1 0 0 0];
%     C                   = spm_bsplinc(img,bsd);
%     img                 = spm_bsplins(C,x0,y0,z0,bsd);
%     img(~isfinite(img)) = 0;
%     
%     ndim                = size(img);
    ...
    % Write isotropic image
%     [pth,nam,ext] = fileparts(fname);
%     nfname        = fullfile(pth,['iso' nam ext]);
