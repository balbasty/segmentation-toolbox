...
for j=1:niter
    ...
    for i=1:size(b,3)
        ...
        sinp = double(sum(P(:,:,i,ix_sin),4));
        lesp = double(sum(P(:,:,i,ix_les),4));
        calp = double(sum(P(:,:,i,ix_cal),4));
        
        bp = double(b(:,:,i))/255;
        bp = (bp>th).*(wp + gp + sinp + lesp + calp);
...
if niter2>0
    ...
    for j=1:niter2
        for i=1:size(b,3)
            ...
            bp = double(c(:,:,i))/255;
            bp = (bp>th).*(wp + gp + cp + sinp + lesp + calp);
...
clear gp wp cp sinp lesp calp bp
