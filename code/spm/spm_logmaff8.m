function [M,ll,h] = spm_logmaff8(varargin)
...
%==========================================================================
function [M,ll,h0] = affreg(buf,MG,x,ff,logtpm,M,regtyp,maxit)
...
for iter=1:200
    ...
    for search = 1:nsearch,
       ...
       %drawnow
        ...
        for i=1:length(x3)
            ...
            y2    = y2(msk);
            y3    = y3(msk);
            b     = spm_sample_logpriors8(logtpm,y1,y2,y3);
            buf(i).b    = b;
            buf(i).msk1 = msk;
        ...
        ll1 = 0;
        for subit=1:32
            h0  = zeros(256,numel(logtpm.dat))+eps;
            if ~rem(subit,4),
                ll0 = ll1;
                ll1 = 0;
            end
    ...
    for i=1:length(x3)   % Loop over slices
        ...
        if nz
            ...
            dmi2  = zeros(nz,1);
            dmi3  = zeros(nz,1);
            [b, db1, db2, db3] = spm_sample_logpriors8(logtpm,y1,y2,y3);
