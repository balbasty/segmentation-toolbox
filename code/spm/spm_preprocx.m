...
cl      = cell(length(z0),1);
buf     = struct('f',cl,'dat',cl,'bf',cl,'code',cl);
buf.msk = cell(length(z0),N);
buf.nmn = cell(length(z0),N);
for z=1:length(z0)
    if ~dotpm && d(3)>1
    ...
    tmp = 0;
    for n=1:N
        buf(z).nmn{n} = sum(buf(z).msk{n}(:));  % Voxels in each channel
        tmp           = tmp + buf(z).msk{n}(:);
    end
    buf(z).nm = sum(tmp>0); % Voxels in all channels
    clear tmp
...
for iter=1:niter
    ...
    if ~isempty(fig{2})                
        [x1,y1,z1] = defs(Twarp,zix,x0,y0,z0,M,false);
        b          = spm_sample_logpriors8(logtpm,x1,y1,z1);
        clear x1 y1 z1
        ...
        for i=1:Kb            
            subplot(K1,K2,i);
            imagesc(b{i}'); axis image xy off; colormap(gray);
        end 
        clear b
        drawnow
    end
    ...
    if iter==1                  
        if use_mog
            ...
            if ~logtpm.uniform
                ...
                mom = mom_struct(K,N);  
                for z=1:length(z0)
                    mom = moments(mom,buf(z));
                end
            else
                % Use a k-means algorithm to estimate moments
                %------------------------------------------------------
                mom = spm_kmeans2mom(buf,K);
            end
...
%                 ll  = ll + lb;
                ...
                for z=1:length(z0)
                    ...
                    % Calculate multinomial part
                    B = double(buf(z).dat);
                    s = 1./(B*wp');
                    
                    % Calculate denominator part of weight (wp) update
                    mgm = mgm + s'*B;
                                        
                    % Calculate responsibilities
                    [q,dll] = latent(buf(z).f,buf(z).bf,mg,mn,vr,po,B,lkp,wp,buf(z).msk,buf(z).code,vb);                    
                    ll      = ll + dll;                                        
        ...
        if ~isempty(fig{1}) && use_mog   
            q  = latent(buf(zix).f,buf(zix).bf,mg,mn,vr,po,double(buf(zix).dat),lkp,wp,buf(zix).msk,buf(zix).code,vb);            
            K1 = floor(sqrt(K));
            K2 = ceil(K/K1); 
            set(0,'CurrentFigure',fig{1});                    
            for i=1:K      
                subplot(K1,K2,i);
                imagesc(reshape(q(:,i),d(1:2))'); axis image xy off; title(['k=' num2str(lkp(i))]); colormap(gray);
            end 
            clear q
            drawnow
        end
        ...
        if dobias
            ...
            for subitbf=1:nsubitbf
                for n=1:N
                    ...
                    if d3>0
                        ...
                        for z=1:length(z0)
                            if ~buf(z).nmn{n}, continue; end

                            if use_mog              
                                q = latent(buf(z).f,buf(z).bf,mg,mn,vr,po,double(buf(z).dat),lkp,wp,buf(z).msk,buf(z).code,vb);
                                
                                cr = cell(N,1);
                                for n1=1:N
                                    cr{n1} = double(buf(z).f{n1}).*double(buf(z).bf{n1}); 
                                end
                                ...
                                for k=1:K
                                    ...
                                    for n1=1:N
                                        w0(buf(z).msk{n1}) = w0(buf(z).msk{n1}) + pr_bf(n1,n,k)*(mn_bf(n1,k) - cr{n1});
                                    end
                                    w1  = w1 + qk.*w0;
                                    w2  = w2 + qk*pr_bf(n,n,k);
                                end
                                wt1 = zeros(d(1:2));
                                wt1(buf(z).msk{n}) = -(1 + cr{n}.*w1(buf(z).msk{n})); % US eq. 34 (gradient)
                                wt2 = zeros(d(1:2));
                                wt2(buf(z).msk{n}) = cr{n}.*cr{n}.*w2(buf(z).msk{n}) + 1; % Simplified Hessian of US eq. 34                               
                            else
                                ...
                                for k1=1:Kb
                                    ...
                                    wt1(buf(z).msk{n}) = wt1(buf(z).msk{n}) - qk.*(gr1.*cr0 + 1);
                                   %wt2(buf(z).msk) = wt2(buf(z).msk) - qk.*(gr1.*cr0 + gr2.*cr0.^2);
                                    wt2(buf(z).msk{n}) = wt2(buf(z).msk{n}) + qk.*(1 - gr2.*cr0.^2);
                                end
                            end
                            clear cr
                ...
                if subitbf > 1 && ~(abs(ll-oll)>tol1*nm)
            ...
            if ~isempty(fig{3})                
                ...
                for i=1:N
                    tmp                  = zeros(d(1:2),'single');
                    tmp(buf(zix).msk{i}) = buf(zix).bf{i};
    ...
    if dodef
        ...
        if use_mog
            if 0%vb
                ...
                dll = objval_vbmog(buf,N,K,Kb,lkp,x0,y0,z0,Twarp,M,logtpm,mg,po,pr,wp,tiny,domiss);
                ll  = ll + dll;
            else
                for z=1:length(z0)
                    if ~buf(z).nm, continue; end
        ...
        for subitdef=1:nitdef
            ...
            for z=1:length(z0)
                ...
                tmp = dp2./p; dp2 = tmp;
                tmp = dp3./p; dp3 = tmp;
                dp1 = reshape(dp1,d(1:2)); 
                dp2 = reshape(dp2,d(1:2)); 
                dp3 = reshape(dp3,d(1:2)); 
...
%                 else
%                     dll = objval_vbmog(buf,N,K,Kb,lkp,x0,y0,z0,Twarp1,M,logtpm,mg,po,pr,wp,tiny,domiss);
%                     ll1 = ll1 + dll;
%                 end

                if ll1<ll
                    % Still not better, so keep searching inwards.                    
                    my_fprintf('Warp:\t%g\t%g\t%g :o(\t(%g)\n',ll1,llr1,llrb,armijo,verbose);
                    armijo = armijo*0.75;
                else
...
if nargout>1 && dotpm && use_mog         
    ...
    if ~isempty(fig{4}) 
        ...
        for i=1:Kb
            subplot(2,Kb,i);
            imagesc(lik(:,:,zix,i)'); axis image xy off; colormap(gray);
        end  
    end
    ...
    % Solve dL/dmu=0 for mu (where mu is the template)  
    %-----------------------------------------------------------
    
    % logs of template
    a1 = zeros([Kb prod(dtpm)],'single'); 
    for k=1:Kb
        a1(k,:) = reshape(logtpm.dat{k},[1 prod(dtpm)]);
    end
    clear logtpm
        
    % logs of mixing weights
    c1  = log(wp)'; 
...
%=======================================================================
function [Q,ll] = latent(f,bf,mg,mn,vr,po,B,lkp,wp,msk,code,vb)
B = log_spatial_priors(B,wp,lkp,mg,vb);
Q = log_likelihoods(f,bf,mg,mn,vr,po,msk,code,vb);
...
%=======================================================================
function B = log_spatial_priors(B,wp,lkp,mg,vb)
B = bsxfun(@times,B,wp);
B = bsxfun(@times,B,1./sum(B,2));
% if vb
...
%==========================================================================
function lb = objval_vbmog(buf,N,K,Kb,lkp,x0,y0,z0,Twarp,M,lntpm,mg,po,pr,wp,tiny,domiss)
lb  = 0;
mom = mom_struct(K,N,domiss,tiny); 
for z=1:length(z0)
    if ~buf(z).nm, continue; end
    ...
    for k=1:K,
        mom.s0(k)     = mom.s0(k)     + sum(q(:,k));
        mom.s1(:,k)   = mom.s1(:,k)   + (q(:,k)'*cr)';
        mom.S2(:,:,k) = mom.S2(:,:,k) + (repmat(q(:,k),1,N).*cr)'*cr;
    end
    clear cr q                                                                       
end

dlb = lowerbound(mom,po,pr);                    
lb  = lb + dlb; 
%==========================================================================
      
%=======================================================================
function [x1,y1,z1] = defs(Twarp,z,x0,y0,z0,M,vec)
if nargin<7, vec = true; end
...
%=======================================================================
function mom = moments(mom,buf,Q)
if nargin<3, Q = buf.dat; end
