function [s,ds1,ds2,ds3] = spm_sample_logpriors8(lntpm,x1,x2,x3)
...
if nargout<=1,
    tot = zeros(dx);
    for k=1:Kb,
        a    = spm_bsplins(double(lntpm.dat{k}),x1,x2,x3,[deg deg deg  0 0 0]);
    ...
    tot(msk) = 1;
    for k=1:Kb,
        s{k}(msk) = exp(double(lntpm.bg2(k)));
        s{k}      = s{k}./tot;
    end
else
    ds1 = cell(1,Kb);
    ...
    da3      = zeros(dx);
    for k=1:Kb,
         s{k}(msk) = exp(double(lntpm.bg1(k)));
         s{k}      = s{k}./tot;
         da1       = da1 + s{k}.*ds1{k};
         da2       = da2 + s{k}.*ds2{k};
         da3       = da3 + s{k}.*ds3{k};
