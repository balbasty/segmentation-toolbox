function [mu,a,L] = spm_shoot_blur_wp(resp,log_wp,prm,its,mu)
...
%                 t(:,:,:,k,cnt) = single(Nii.dat(:,:,:));
%             end
%             log_wp(cnt,:)  = log(obj.wp);
% 
%             cnt = cnt + 1;
%         end
%     end
...
if nargin<3, prm = [1,1,1, 0.01 0.02 1];       end; % Default regularisation
if nargin<4, its = 16;                         end; % Maximum no. iterations

S = size(resp,5); % Number of subjects
d = [size(mu),1,1,1];
