function obj = spm_preproc8_new(obj,tpm,fig)
...
nsubit    = obj.nsubit;
uniform   = obj.uniform;
if uniform, obj.do_def = false; end
do_bf     = obj.do_bf;
do_def    = obj.do_def;
...
%-----------------------------------------------------------------------
[buf,nm,vr0] = init_buf(N,obj,V,x0,y0,z0,o,M,tpm,tot_S);

% Initialise tissue weights
%-----------------------------------------------------------------------
...
if isfield(obj,'gmm') && isfield(obj,'mg')
    gmm      = obj.gmm;
    mg       = obj.mg;            
else                        
    [gmm,mg] = init_gmm(gmm,Kb,N,buf,obj,uniform,modality,ix_subj);                        
    lkp      = 1:Kb;            
end
...
for iter=1:niter
    ...
    for subit=1:nsubit
        ...
        %------------------------------------------------------------
        [ll,mg,gmm,wp,L] = update_gmm(ll,llr,llrb,buf,mg,gmm,wp,lkp,wp_reg,iter,tol1,nm,nitgmm,fig,L,print_ll);
 
        debug_view('responsibilities',fig{1},lkp,buf,gmm,mg,wp);
        ...
        if do_bf
            ...
            % faster.
            %------------------------------------------------------------
            [ll,llrb,buf,chan,L,armijo(1)] = update_bf(ll,llrb,llr,buf,mg,gmm,wp,lkp,chan,fig,L,print_ll,armijo(1));
    ...
    if do_def
        for subit=1:nsubit
            ...
            %------------------------------------------------------------
            [ll,llr,buf,Twarp,L,armijo(2)] = update_def(ll,llrb,llr,buf,mg,gmm,wp,lkp,Twarp,sk4,M,MT,tpm,x0,y0,z0,param,iter,fig,L,print_ll,tot_S,armijo(2));
        end    
    end
