function [ll,llrb,buf,chan,L,armijo] = update_bf(ll,llrb,llr,buf,mg,gmm,wp,lkp,chan,fig,L,print_ll,armijo)
nz = numel(buf);
N  = numel(buf(1).f);
K  = numel(mg);
d  = size(buf(1).msk);
...
for n=1:N
    ...
    if d3>0
        ...
        for z=1:nz
            ...
            q = latent(buf(z).f,buf(z).bf,mg,gmm,buf(z).dat,lkp,wp,buf(z).code,cr);
        ...
        for line_search=1:12
            ...
            [mom,dll] = compute_moments(buf,K,mg,gmm,wp,lkp);        
            ll        = ll + dll; 
